...
export const RelatedPosts:FC<BlogPostProp> = ({blogPosts}) => {
  return (
    <div>
        {blogPosts.map(
            ({ title, sampleImg, date, author, postType, id }, i) => {
              return (
                <Link to={`/blog/posts/${title}/${id}/${postType}`} key={i}>
                  <div className="overflow-hidden cursor-pointer group mb-4 h-[120px] sm:h-[130px] md:h-[140px] flex  items-center bg-white border border-gray-200 rounded-lg shadow flex-row w-full hover:bg-gray-100 ">
